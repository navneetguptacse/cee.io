...
func (h *Handler) GenerateKey(w http.ResponseWriter, r *http.Request) {
	...
	if !auth.CanGenerate(caller, req.Type, req.Role) {
		respondError(w, http.StatusForbidden, "Forbidden", "You do not have permission to generate this credential type and role")
		return
	}
...
func (h *Handler) ListKeys(w http.ResponseWriter, r *http.Request) {
	...
	if caller == nil || caller.Type != auth.TypeAuth || caller.Role != auth.RoleMaster {
		respondError(w, http.StatusForbidden, "Forbidden", "Master AUTH API key required to view keys")
		return
	}
...
func (h *Handler) RevokeKey(w http.ResponseWriter, r *http.Request) {
	...
	caller := GetKeyFromContext(r.Context())
	if caller == nil || caller.Type != auth.TypeAuth || caller.Role != auth.RoleMaster {
		respondError(w, http.StatusForbidden, "Forbidden", "Master AUTH API key required to revoke keys")
		return
	}
